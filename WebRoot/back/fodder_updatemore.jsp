...
function addaline(){
	...
	bzitem.find('.m-articleid').text('0');
	bzitem.find('.m-img').attr('src','res/ddspic.png');
	$('.sub-add').before(bzitem);
}

function addahistoryline(id,title,picurl,descrition,url){
	console.log("addahistoryline");
	var theitem = $('.msg-item-wrapper').find('.appmsgItem').eq(1);
	var bzitem = theitem.clone();
	bzitem.find('.m-title').text(title);
	bzitem.find('.m-imgurl').text(picurl);
	bzitem.find('.m-articleid').text(id);
	bzitem.find('.m-con').text(descrition);
	bzitem.find('.m-url').text(url);
	bzitem.find('.m-img').attr('src',picurl);
	$('.sub-add').before(bzitem);
}

function addfirstloadline(id,title,picurl,descrition,url){
	window.dqdiv = $('#appmsgItem1');
	$('.msg-editer-wrapper').css('marginTop','40px');
		//把初始值传入文本框
		$('#title').val(title);
		...
		$('#con').val(descrition);
		$('#imgurl').val(picurl);
		window.dqdiv.find('.m-img').attr('src',picurl);
		//在隐藏的地方设置好初始值
		window.dqdiv.find('.m-title').text(title);
		window.dqdiv.find('.m-url').text(url);
		window.dqdiv.find('.m-con').text(descrition);
		window.dqdiv.find('.m-imgurl').text(picurl);
		window.dqdiv.find('.m-articleid').text(id);
}

function addsecondline(id,title,picurl,descrition,url){
	var theitem = $('.msg-item-wrapper').find('.appmsgItem').eq(1);
	window.dqdiv = theitem;
	//把初始值传入文本框
		$('#title').val(title);
		$('#url').val(url);
		$('#con').val(descrition);
...
function delitem(o){
	if(window.dqdiv[0] == window.curedt[0]){
		...
		$('#con').val(window.dqdiv.find('.m-con').text());
		$('#imgurl').val(window.dqdiv.find('.m-imgurl').text());
		//$('#articleid').val(window.dqdiv.find('.m-articleid').text());
		//console.log("1="+$('#title').val());
		//console.log("2="+window.dqdiv.find('img').attr('src'));
		window.dqdiv.find('.m-img').attr('src',window.dqdiv.find('.m-imgurl').text());
		//window.kindeditor_fg.html(window.dqdiv.find('.m-con').html());
...
function savedate(){
	if(czjy()){
		var fodderid = <s:property value="fodder.id"/>;
		var data = [];
		var isenter = true;
		$('.appmsgItem').each(function(){
			...
			sd.id = $.trim($(this).find('.m-articleid').text());
		...
		if(isenter){
			loading('数据保存中...');
			ajaxjson('fodderAction!updatemore',{ fodderid:fodderid,data:data},function(m){
...
function loadAll(){
	...
	var size = <s:property value="fodderarticles.size()"/>;
	var articlelist = new Array(size);
	<s:iterator value="fodderarticles" var="fodderarticle" status="index2">
		articlelist[<s:property value="#index2.count-1"/>]= new Array(5);
		articlelist[<s:property value="#index2.count-1"/>][0]=<s:property value="id"/>;
		articlelist[<s:property value="#index2.count-1"/>][1]='<s:property value="title"/>';
		articlelist[<s:property value="#index2.count-1"/>][2]='<s:property value="picurl"/>';
		articlelist[<s:property value="#index2.count-1"/>][3]='<s:property value="description"/>';
		articlelist[<s:property value="#index2.count-1"/>][4]='<s:property value="url"/>';
		<s:if test="#index2.count==1">
			//填充头部大图文信息
			addfirstloadline(<s:property value="id"/>,'<s:property value="title"/>','<s:property value="picurl"/>','<s:property value="description"/>','<s:property value="url"/>');
		</s:if>
		<s:elseif test="#index2.count==2">
			addsecondline(<s:property value="id"/>,'<s:property value="title"/>','<s:property value="picurl"/>','<s:property value="description"/>','<s:property value="url"/>');
		</s:elseif>
		<s:else>
			addahistoryline(<s:property value="id"/>,'<s:property value="title"/>','<s:property value="picurl"/>','<s:property value="description"/>','<s:property value="url"/>');
		</s:else>
			...
			<div class="span5 msg-preview" id="nrdiv1">
				...
				<div class="msg-item-wrapper" relid="">
					<div id="appmsgItem1" class="appmsgItem msg-item">
						...
						<div rel="articleid" class="m-articleid" style="display: none;"></div>
					</div>
