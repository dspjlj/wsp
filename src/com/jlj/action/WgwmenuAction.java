...
SessionAware,ServletResponseAware,ServletRequestAware {
	...
	public String list() throws Exception{
		...
		//总记录数
		totalCount=wgwmenuService.getTotalCount(con,convalue,status,publicaccount);
		//总页数
		pageCount=wgwmenuService.getPageCount(totalCount,size);
		if(page>pageCount&&pageCount!=0){
			page=pageCount;
		}
		//所有当前页记录对象
		wgwmenus=wgwmenuService.queryList(con,convalue,status,publicaccount,page,size);
	 ...
	 */
	public String view(){
		wgwmenu=wgwmenuService.loadById(id);
		return "view";
	}
	/**
	 * 跳转到修改页面
